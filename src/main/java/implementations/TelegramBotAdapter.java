...
public class TelegramBotAdapter extends TelegramLongPollingBot {
    ...
    private void handleButtonClick(Long chatId, String buttonText) {
        switch (buttonText) {
            case "Поиск игры":
                ...
                sendTextMessage(chatId, "Введите название игры:", false);
                break;
            case "Информация по ID":
                userStates.put(chatId, "waiting_for_appid");
                sendTextMessage(chatId, "Введите AppID:", false);
                break;
            case "История":
                String history = processor.processCommand(chatId, "history", "");
                sendResponse(chatId, history, "history");
                break;
            case "Помощь":
                String help = processor.processCommand(chatId, "help", "");
                sendResponse(chatId, help, "help");
                break;
            case "Главное меню":
                String start = processor.processCommand(chatId, "start", "");
                sendResponse(chatId, start, "start");
                break;
        }
    }
    ...
    private void handleRegularMessage(Long chatId, String messageText) {
        ...
        TelegramInputParser.ParsedCommand parsed = inputParser.parse(messageText);
        String response = processor.processCommand(chatId, parsed.getCommand(), parsed.getArgument());
        sendResponse(chatId, response, parsed.getCommand());
    }
    ...
    private void handleUserInput(Long chatId, String userInput, String state) {
        try {
            ...
            if (state.equals("waiting_for_search")) {
                response = processor.processCommand(chatId, "search", userInput);
                sendResponse(chatId, response, "search");
            } else if (state.equals("waiting_for_appid")) {
                response = processor.processCommand(chatId, "info", userInput);
    ...
    private void sendResponse(Long chatId, String response, String command) {
        ...
        boolean showKeyboard = command.equals("start") || command.equals("help") || command.equals("history");
    ...
    private void sendTextMessage(Long chatId, String text, boolean showKeyboard) {
        ...
        if (showKeyboard) message.setReplyMarkup(createMainKeyboard());
    ...
    private void sendPhotoWithCaption(Long chatId, String imageUrl, String caption, boolean showKeyboard) {
        try {
            ...
            if (showKeyboard) photo.setReplyMarkup(createMainKeyboard());
            execute(photo);
        } catch (Exception e) {
            sendTextMessage(chatId, caption, showKeyboard);
        }
    }
